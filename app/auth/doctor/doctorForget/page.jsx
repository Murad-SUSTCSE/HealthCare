...
export default function DoctorForgotPasswordMinimal() {
  ...
  const router = useRouter()
	...
	const handleSend = async (e) => {
		...
		try {
			...
			const res = await fetch('/api/auth/doctor/forgot-password/request', {
				method: 'POST',
			...
			// setStatus('sent')
            router.push(`/auth/doctor/doctorReset?email=${encodeURIComponent(email)}`)
		} catch (err) {
				...
				<div className="p-8 md:p-10 space-y-8">
					<header className="space-y-2 text-center">
						...
						<p className="text-sm md:text-base text-green-700/70 max-w-md mx-auto">
							Enter the email associated with your doctor account. If it exists, we'll send a 6‑digit verification code to reset your password.
						</p>
					</header>
					...
					<form onSubmit={handleSend} className="space-y-6">
						...
						{/* {status === 'sent' && (
							<div className="rounded-md border border-green-300 bg-green-50 text-green-800 px-4 py-3 text-sm animate-fade-in">
								...
								<Link href={`/auth/doctor/reset-password?email=${encodeURIComponent(email)}`} className="ml-1 underline font-semibold text-teal-700 hover:text-teal-800">
									Reset Password
								</Link>
							</div>
						)} */}
					...
					<div className="text-center text-sm text-green-900/70">
						<p>
							...
							<Link href="/auth/doctor/login" className="font-medium text-teal-700 hover:text-teal-800 underline underline-offset-4">
								Back to Login
							</Link>
						</p>
						<p className="mt-2">
